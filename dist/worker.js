/**
 * AegisForge MicroWebM Worker Muxer
 * Serialized as a string to be dynamically loaded as a Blob URL without separate file requests.
 */
...
const AegisMuxer = (() => {
    ...
    class MemSink {
        ...
        write(d) { if (d && d.byteLength) { this.chunks.push(d); this.len += d.byteLength; } }
        get buffer() {
            const b = new Uint8Array(this.len); let o = 0;
            for (let i = 0; i < this.chunks.length; i++) { b.set(this.chunks[i], o); o += this.chunks[i].byteLength; }
            return b.buffer;
        }
    }
    ...
    class StreamSink {
        ...
        write(d) { if (d && d.byteLength) { this.cb(d, this.pos); this.pos += d.byteLength; } }
    }
    ...
    class FileSink {
        ...
        write(d) {
            ...
            try {
                ...
                if (this.errCb) this.errCb(new Error("FileSink IO Error: " + e.message));
            }
        }
        close() {
            try {
    ...
    class WebStreamSink {
        ...
        write(d) {
            ...
            try {
                ...
                if (this.errCb) this.errCb(new Error("WebStreamSink IO Error: " + e.message));
            }
        }
        close() {
    ...
    class Scribe {
        ...
        bytes(d) { if (!d || !d.byteLength) return; this.ensure(d.byteLength); this.buf.set(d, this.p); this.p += d.byteLength; }
        zero(n) { if (n <= 0) return; this.ensure(n); this.buf.fill(0, this.p, this.p + n); this.p += n; }
        ...
        ebm(hex) { for (let i = 0; i < hex.length; i += 2) this.u8(parseInt(hex.substring(i, i + 2), 16)); this.ensure(8); const s = this.p; this.p += 8; this.stack.push({ s, m: 'e' }); }
        ebu(hex) { for (let i = 0; i < hex.length; i += 2) this.u8(parseInt(hex.substring(i, i + 2), 16)); this.ensure(8); this.buf[this.p++] = 0x01; for (let i = 0; i < 7; i++) this.buf[this.p++] = 0xff; }
    ...
    class Engine {
            ...
            try {
                if (this.fmt === "mp4" || this.fmt === "mov") {
                    ...
                    if (this.at) {
                        ...
                        this.sc.ebm("86"); this.sc.str(this.at.codec.includes("opus") ? "A_OPUS" : "A_AAC"); this.sc.end();
        ...
        _flushSc() {
            if (this.sc.p > 0) {
                ...
                try { this.sink.write(d); } catch (e) { this.err(e); }
        ...
        addVideo(chunk, meta) {
            ...
            try {
                ...
                if (chunk.copyTo) chunk.copyTo(raw); else raw.set(new Uint8Array(chunk));

                if (meta?.decoderConfig) {
                    if (meta.decoderConfig.description && !this.vt.cfgData) this.vt.cfgData = new Uint8Array(meta.decoderConfig.description);
            ...
            } catch (e) { console.warn("[AegisMuxer] Recovered from corrupted video chunk: ", e); }
        }
        ...
        addAudio(chunk, meta) {
            ...
            try {
                ...
                if (meta?.decoderConfig?.description && !this.at.cfgData) this.at.cfgData = new Uint8Array(meta.decoderConfig.description);
                this._push(this.at, raw, true, ts, ts, dur, 0);
            } catch (e) { console.warn("[AegisMuxer] Recovered from corrupted audio chunk: ", e); }
        }

        _push(trk, data, isKey, pts, dts, dur, cto) {
            if (this.tBase === -1) this.tBase = Math.min(pts, dts);
            ...
            for (let t of [this.vt, this.at].filter(Boolean)) {
                ...
                if (this.fmt === "webm" || this.fmt === "mkv") {
                    ...
                    if (shouldCluster) {
                        ...
                        this.sc.ebm("E7"); this.sc.u32(tc); this.sc.end(); // Timecode
                        this.wClus = t.queue[0].p;
                    }
                    ...
                    while (t.queue.length) {
                        ...
                        if (relTs < 0) relTs = 0; else if (relTs > 32767) relTs = 32767;
                        this.sc.ebm("A3"); this.sc.ebv(t.id); this.sc.i16(relTs); this.sc.u8(f.k ? 0x80 : 0x00); this.sc.bytes(f.d); this.sc.end();
                    ...
                    while (t.queue.length) {
                        let f = t.queue.shift();
                        this.sc.rif(t.id === 1 ? "00dc" : "01wb"); this.sc.bytes(f.d); this.sc.end();
                        f.d = null;
                    }
                }
            }
        ...
        _checkFrag() {
            if (this.fmt !== "mp4" && this.fmt !== "mov") { 
                // Don't auto-flush Interleaved logic aggressively every frame 
                // Wait until queue builds up a bit to batch Clusters properly
                const ptrk = this.vt || this.at;
                if (ptrk && ptrk.queue.length && ptrk.queue[ptrk.queue.length - 1].p - ptrk.queue[0].p >= ptrk.scale * 2) {
                    this._flushInterleaved(); 
                }
                return; 
            }
        ...
        _writeFrag() {
            ...
            for (let t of tks) {
                this.sc.box("traf");
                this.sc.box("tfhd"); this.sc.u32(0x20020); this.sc.u32(t.id); this.sc.end();
            ...
            for (let x of trunOffs) {
                ...
                for (let f of x.t.queue) mdatOffset += f.d.byteLength;
            }
            this._flushSc();

            this.sc.u32((mdatOffset - moofSize - 8) + 8); this.sc.str("mdat"); this._flushSc();

            for (let t of tks) {
                for (let f of t.queue) {
                    try { this.sink.write(f.d); } catch (e) { this.err(e); }
                    f.d = null;
                }
                t.queue.length = 0;
            }
            ...
            try {
                if (this.opt.mode === "fragmented") {
                    if (this.fmt === "mp4" || this.fmt === "mov") {
                        ...
                        this._flushInterleaved();
                        if ((this.fmt === "webm" || this.fmt === "mkv") && this.wClus !== -1) {
                             this.sc.end(); // close the final open Cluster
                             this._flushSc();
                        }
                    }
        ...
        _writeTrak(t, isFrag) {
            ...
            if (!isFrag) {
                ...
                this.sc.box("stsz"); this.sc.u32(0); this.sc.u32(0); this.sc.u32(t.stsz.length); for (let s of t.stsz) this.sc.u32(s); this.sc.end();
...
let ve = null, ae = null, mx = null, sink = null;
// Auto-Recovery Helpers
let veConfig: any = null;
let aeConfig: any = null;

function reviveVideo() {
    if (ve) { try { ve.close(); } catch(e){} }
    ve = new VideoEncoder({
        output: (c, meta) => mx.addVideo(c, meta),
        error: (err) => {
            console.warn("[AegisMuxer] VideoEncoder Error, attempting auto-revival...", err);
            reviveVideo();
        }
    });
...
function reviveAudio() {
    if (ae) { try { ae.close(); } catch(e){} }
    ae = new AudioEncoder({
        output: (c, meta) => mx.addAudio(c, meta),
        error: (err) => {
            console.warn("[AegisMuxer] AudioEncoder Error, attempting auto-revival...", err);
            reviveAudio();
        }
    });
...
self.onmessage = async (e) => {
    ...
    try {
        if (type === 'init') {
            let directToDisk = payload.directToDisk || false;
            let formatStr = payload.mp4Container ? 'mp4' : 'webm';
            
            if (payload.stream) {
                sink = new AegisMuxer.WebStreamSink(payload.stream, err => self.postMessage({ type: 'error', error: err.message }));
                ...
                sink = new AegisMuxer.FileSink(opfsH, err => self.postMessage({ type: 'error', error: err.message }));
            } else {
            ...
            mx = new AegisMuxer.Engine({
                ...
                onError: err => self.postMessage({ type: 'error', error: err.message })
            });
            ...
            try {
                if (ve.state !== 'configured') reviveVideo();
                ve.encode(payload.frame, { keyFrame: payload.keyFrame });
            } catch(err) {
                console.warn("[AegisMuxer] Dropping frame and reviving VideoEncoder...", err);
                reviveVideo();
            }
            try { payload.frame.close(); } catch(e) {}
            self.postMessage({ type: 'queue-capacity', active: ve.encodeQueueSize < 10 });
        } else if (type === 'encode-audio') {
            try {
                if (ae.state !== 'configured') reviveAudio();
                ae.encode(payload.audioData);
            } catch(err) {
                console.warn("[AegisMuxer] Dropping audio chunk and reviving AudioEncoder...", err);
                reviveAudio();
            }
            try { payload.audioData.close(); } catch(e) {}
        } else if (type === 'flush') {
            const ar = [];
            ...
            if (ae) ae.close();
            if (mx) mx.finalize();
            if (sink instanceof AegisMuxer.FileSink || (AegisMuxer.WebStreamSink && sink instanceof AegisMuxer.WebStreamSink)) {
                sink.close();
                const dummy = new ArrayBuffer(0);
                self.postMessage({ type: 'done', buffer: dummy }, [dummy]);
            } else {
                const b = sink.buffer;
                self.postMessage({ type: 'done', buffer: b }, [b]);
            }
        }
    } catch (err: any) {
